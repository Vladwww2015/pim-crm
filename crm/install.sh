...
function cleanTemporaryFiles() {
    if [ -f "${scriptDirectory}/espocrm-installer-2.1.0.zip" ]; then
        rm "${scriptDirectory}/espocrm-installer-2.1.0.zip"
    fi

    if [ -d "${scriptDirectory}/espocrm-installer-2.1.0" ]; then
        rm -rf "${scriptDirectory}/espocrm-installer-2.1.0"
    fi
}
...
function downloadSourceFiles() {
    rm -rf ./espocrm-installer-2.1.0.zip ./espocrm-installer-2.1.0/

    download https://github.com/espocrm/espocrm-installer/releases/download/2.1.0/espocrm-installer-2.1.0.zip "espocrm-installer-2.1.0.zip"
    unzip -q "espocrm-installer-2.1.0.zip"

    if [ ! -d "./espocrm-installer-2.1.0" ]; then
...
actionCommand() {
    ...
    cp ./espocrm-installer-2.1.0/commands/command.sh "${data[homeDirectory]}/command.sh" || {
